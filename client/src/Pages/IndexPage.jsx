...
const IndexPage = () => {
  ...
  // Hardcoded conversion rate (INR to USD)
  const exchangeRate = 75; // 1 USD = 75 INR (example rate)

  const fetchPlaces = async (currentPage) => {
    try {
      setLoadingMore(true);
      const { data } = await axios.get(`http://localhost:5001/places?page=${currentPage}`);
      if (data.length === 0) {
        setAllFetched(true);
      }
  ...
  const fetchSearchResults = async (query) => {
    try {
      setLoading(true);
      setIsSearching(true);
      const { data } = await axios.get(`http://localhost:5001/places?query=${query}`);
  ...
  const convertInrToUsd = (inr) => {
    return (inr / exchangeRate).toFixed(2); // Converting INR to USD
  };
      ...
      <div className="grid grid-cols-1 justify-items-center py-4 px-3 gap-y-8 sm:grid-cols-2 sm:gap-x-4 md:grid-cols-2 md:gap-x-4 lg:grid-cols-3 lg:gap-x-6 xl:grid-cols-4 2xl:grid-cols-5 mt-3">
        {(loading) ?
          ...
          currentList?.map((place) => {
            const priceInUsd = convertInrToUsd(place.price); // Convert price to USD
            return (
              <Link
                to={`/place/${place._id}`}
                key={place._id}
              ...
              >
                ...
                <div className="font-semibold">
                  <span className="font-bold">
                    ${priceInUsd} {/* Display price in USD */}
                  </span>{" "}
                  per night
                </div>
              </Link>
