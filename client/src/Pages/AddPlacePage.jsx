...
const AddPlacePage = () => {
  ...
  const [price, setPrice] = useState(""); // Price in USD
  const descRef = useRef();

  useEffect(() => {
    if (id) {
      console.log(id);
      axios.get(`http://localhost:5001/place/${id}`).then(({ data }) => {
        setTitle(data.title);
  ...
  const addNewPlace = async (e) => {
    ...
    try {
      if (addedPhotos.length < 3) {
        toast.info("Upload at least 3 photos for your place");
        return;
      }
      ...
      if (id) {
        await axios.put("http://localhost:5001/place", { id, ...data });
        toast.success("Place Updated Successfully!!");
      } else {
        await axios.post("http://localhost:5001/place", data);
  ...
  return (
    <form onSubmit={addNewPlace} className="px-2 md:px-8">
      ...
      <InputDesc text="Title for your place, should be short and catchy as in advertisement" isRequired={true} />
      <input
        type="text"
      ...
      <InputDesc text="Add as many photos as you can for better evaluation and judgement" isRequired={true} />
      <PhotosUploader
        photoLink={photoLink}
      ...
      <InputDesc text={"Description of the place (should be of at least 50 words)"} isRequired={true} />
      <textarea
        rows={5}
        className="w-full rounded-xl py-1.5 px-4 border border-gray-300 mb-4"
        value={description}
      ...
      <div className="flex flex-col mt-4 w-full gap-x-16 md:flex-row">
        ...
        <div className="w-full md:w-1/2">
          <InputHeading text={"Price per night (USD)"} />
          <InputDesc text={"Try to give reasonable price considering all the factors (in USD)"} isRequired={true} />
          <input
            type="number"
            value={price}
            required
      ...
      <button
        className="bg-pink w-full rounded-2xl text-white py-2 font-semibold text-xl mt-3 mb-5 hover:scale-95 transition-all"
        onClick={(e) => addNewPlace(e)}
      >
        Save Your Place
      </button>
    </form>
