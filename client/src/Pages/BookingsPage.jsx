...
const INR_TO_USD = 83; // Conversion rate INR to USD
...
const BookingsPage = () => {
  ...
  const getBookings = async () => {
    try {
      const { data } = await axios.get('http://localhost:5001/booking/account');
      setBookings(data);
      setLoading(false);
    } catch (error) {
      ...
      {(bookings && bookings.length > 0) ? (
        bookings?.map(booking => (
          <Link
            ...
            className="flex gap-3 w-full bg-gray-200 rounded-2xl max-w-[650px] xs:gap-4 md:gap-1"
            key={booking._id}
          >
            <div className="w-48 sm:w-64 h-32 xs:h-34 sm:h-40">
              <Image
                src={booking.place.photos[0]}
                alt="booking-preview-photo"
              ...
              <div className="text-md sm:text-xl">
                ...
                <div className="flex gap-1 mt-[-5px] items-center">
                  ...
                  <span className="text-sm sm:text-xl">
                    Total price: ${((booking.price) / INR_TO_USD).toFixed(2)}
                  </span>
                </div>
              </div>
            </div>
